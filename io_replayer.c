    ...
    while (1) {
	  ...
	  if ((rand() % 100) < ECC_ERROR_PCT) {
	    ...
	    while (tries < MAX_ECC_RETRIES) {
	      useconds_t delay = MIN_ECC_DELAY_US
	                       + (rand() % (MAX_ECC_DELAY_US - MIN_ECC_DELAY_US + 1));
	      usleep(delay);
	      tries++;
	  ...
	  if (reqflag[cur_idx] == READ && (rand() % 100) < INJECT_PCT) {
	    ...
	    for (int i = 0; i < tries; i++) {
	      useconds_t d = (rand() % (MAX_DELAY_US - MIN_DELAY_US + 1))
	                     + MIN_DELAY_US;
	      usleep(d);
	  ...
	  if ((rand() % 100) < WEAR_PCT) {
	    ...
	    useconds_t extra = WEAR_MIN_US
	                     + (rand() % (WEAR_MAX_US - WEAR_MIN_US + 1));
	    usleep(extra);
	  }
	#endif
